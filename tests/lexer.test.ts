...
describe('Lexer: choices', () => {
  ...
  it('tokenizes block choice text delimited by ```', () => {
    ...
    expect(types.filter((t) => t === TokenTypes.CHOICE_TEXT_BOUND).length).toBe(
      2
    );
  });
});
...
describe('Lexer: replicas and inline calls', () => {
  ...
  it('keeps inline {call:...} inside STRING content (parser will split later)', () => {
    ...
    lexer.process();
    const tokens = lexer.getTokens();
    const stringValues = tokens
      .filter((t) => t.type === TokenTypes.STRING)
      .map((t) => String(t.value ?? ''));
    expect(
      stringValues.some((v) => v.includes('{call:say("x, y", nested(2))}'))
    ).toBe(true);
  });
});
