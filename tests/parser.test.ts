...
import { parseProgramFromTokens, parseSimpleStatements, getParserIssues } from '../src/parser'
...
describe('Parser: program sections', () => {
  ...
  it('reports issue if section has no name', () => {
    ...
    expect(issues.find(i => i.message.includes('Expected section name'))).toBeTruthy()
  })
})
...
describe('Parser: simple statements', () => {
  ...
  it('emits error when Goto missing IDENTIFIER', () => {
    ...
    expect(issues.find(i => i.message.includes('Goto must be followed by IDENTIFIER'))).toBeTruthy()
  })
  ...
  it('parses Call with args and warns on empty call', () => {
    ...
    if (withArgs[0].kind === 'Call') expect(withArgs[0].args).toEqual(['1', '"a"'])

    parseSimpleStatements(lex('@call:fn\n'))
    const issues = getParserIssues()
    expect(issues.find(i => i.message.includes('Empty or malformed call'))).toBeTruthy()
  })

  it('parses Replica and splits inline {call:...} segments', () => {
    const stmts = parseSimpleStatements(lex('" Hello {call:say(1, "x")} world\n'))
...
describe('Parser: Choice', () => {
  ...
  it('attaches @tags and @@choice_tags correctly', () => {
    ...
    if (stmts[0].kind === 'Choice') {
      ...
      expect(stmts[0].choiceTags?.find(t => t.name === '@only')?.value).toBe(' true')
    }
  })
})
