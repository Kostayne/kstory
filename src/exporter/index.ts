...
program
  .name('kstory-export')
  .description('Export .ks files to JSON format (simple by default, optimized for game engines)')
  .version('1.0.0');
...
program
  ...
  .option('-f, --full', 'Export full JSON with position information (for LSP/editors)')
  .action(
    async (
      input: string,
      options: { output?: string; pretty?: boolean; verbose?: boolean; full?: boolean }
    ) => {
      try {
        const outputPath = options.output || input.replace(/\.ks$/, '.json');
